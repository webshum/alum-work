...
<section {{ $attributes->merge(['class' => 'action hidden md:block']) }}>
    <div class="center">
        ...
        <ul class="grid grid-cols-[1.3fr_1fr_1fr] md:gap-5 lg:gap-[34px]">
            ...
            <li>
                <a href="{{ $action->url ?? '#' }}" class="rounded-[30px] overflow-hidden flex h-full hover:shadow-[0px_0px_15px_rgba(0,0,0,0.3)]">
                    <img class="object-cover w-full h-full" src="{{ Storage::url($action->image ?? null) }}" alt="{{ $action->title }}" loading="lazy">
                </a>
            </li>
            @endforeach
        </ul>
